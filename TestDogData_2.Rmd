...
for (i in 1:length(filelist)){
  ...
  getPower <- function(column, upper=TRUE){
    ...
    if (upper){
      (CI <- sort(variable)[length(variable)*0.95])
    }
    if (!upper){
      CI <- sort(variable)[length(variable)*0.05]
    }
    
    row <- CI_out <- CI_robust_75 <- CI_robust_99 <- c()
    for (k in 1:nrow(sel_snps)){
        ...
        if(upper){
          CI_out[k] <- sum((variable[cond]) > CI)>5
    ...
    out=data.frame(CI_out, CI_robust_75, CI_robust_99)
    print(out)
    power = colSums(out)/nrow(out)
    power.out = data.frame(cutoff.CI=CI, cutoff.robust75=var_75, 
                     cutoff.robust99=var_99,
                     power.CI=power[1],
                      power.robust75=power[2],
                      power.robust99=power[3]
    )
    return(power.out)
  }
  ### End function
  ...
  a<- lapply(whichcols, getPower)
  a<-data.frame(matrix(unlist(a), ncol = 6, byrow=TRUE))
  ...
  b<-lapply(whichcolsMin, getPower, upper=FALSE)
  b<-data.frame(matrix(unlist(b), ncol = 6, byrow=TRUE))
